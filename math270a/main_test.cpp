...
int main()
{
    ...
    //##############################################################################################
    // 2D TEST for SVD and differentiating SVD and differentiating RS
    //##############################################################################################
    if(0){
        MATRIX_2X2<T> F,U,V,UtU,VtV,UUt,VVt,DeltaF,DeltaU,DeltaV,DeltaSigma,DeltaR,DeltaS,R,S;
        VECTOR_2D<T> sigma,delta_sigma;
        int random_integer;

        for(int test=0;test<100000;test++){
            std::cout<<"test 2d  "<<test<< " \n";
            ...
            F.Delta_RS(DeltaF,DeltaR,DeltaS);
            ...
            MATRIX_2X2<T> FV=F*V;
            MATRIX_2X2<T> RS_restore=DeltaR*S+R*DeltaS;
            MATRIX_2X2<T> RS_error=DeltaF-RS_restore;
            ...
            if(std::abs(U.Determinant()-1)>1e-10 || 
                ...
                (RS_error.Norm() > 1e-8 && std::abs(sigma(0)+sigma(1))>1e-10) ||
                (sigma(0)<0&&sigma(1)<0) || 
    ...
    //##############################################################################################
    if(1){
        MATRIX_3X3<T> F,U,V,UtU,VtV,UUt,VVt,DeltaF,DeltaSigma,DeltaU,DeltaV,DeltaR,DeltaS,R,S;
        ...
        for(int test=0;test<10000000;test++){
            ...
            MATRIX_3X3<T> FV=F*V;
            MATRIX_3X3<T> RS_restore=DeltaR*S+R*DeltaS;
            MATRIX_3X3<T> RS_error=DeltaF-RS_restore;
            ...
            if(
                ...
                vvt_error.Norm()>1e-7 ||
                error.Norm() >1e-7 ||
                (RS_error.Norm()>1e-7 && std::abs(sigma(0)+sigma(1))>1e-10 && std::abs(sigma(0)+sigma(2))>1e-10 && std::abs(sigma(1)+sigma(2))>1e-10) ||
